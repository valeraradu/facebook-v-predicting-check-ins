...
def process_one_cell(cell_train, cell_test, fw, th, n_neighbors, x_min, y_min):
    
    # Remove infrequent places
    places, idx, counts = pd.unique(cell_train[:, -1], return_inverse=True, return_counts=True)
    ...
    preds_all = dict(zip(le.inverse_transform(classes), zip(*y_pred)))
    preds_all = pd.DataFrame.from_dict(preds_all)
    #preds_all['row_id'] = row_ids;
    #preds_all = preds_all.set_index('row_id')

    preds_all.to_csv("./raw/knn-lb-58369/" + raw_output + str(x_min) + "_" + str(y_min) + ".csv", index = False);
