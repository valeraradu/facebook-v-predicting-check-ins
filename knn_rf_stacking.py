...
def process_cell(train_cell, test_cell):
    ...
    preds_train_level1 = fit_predict_proba_2clf(X_1, y_1, X_2).fillna(value=0)
    ...
    diff = set(y_1.unique()) - set(y_2.unique())
    intersect = set(y_1.unique()).intersection(y_2.unique())
    ...
    for pair in intersect:
        ...
        coeficients[pair] = meta_clf.coef_
        summed[pair] = meta_clf.predict_proba(preds_test_level1_small)[:,1]

    #print coeficients

    for pair in set(y.unique()) - intersect:
        col_1 = str(pair)+'_1'
        col_2 = str(pair)+'_2'
        summed[pair] = preds_test_level1[col_1] * 2 + preds_test_level1[col_2] * 3
...
def process_column(x_min, x_max, y_ranges, x_end, y_end, train, test, preds_total):
    ...
    for y_min, y_max in  y_ranges: 
        ...
        preds_total[x_min]  = pd.concat([preds_total[x_min] , chunk], axis=0);
